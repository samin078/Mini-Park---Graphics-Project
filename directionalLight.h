...
public:
    ...
    void setUpDirLight(Shader& lightingShader)
    {
            ...
            lightingShader.setVec3("dirLight.ambient", ambientOn * ambient);  //turn off hoile ambientOn 0, so ultimately ambient = 0 . complemete shutdown, we can add some number to make sure there is little light/color, even if everything is turned off, its called ambient light
